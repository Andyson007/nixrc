{ pkgs, ... }: 
{
  ...
  home.file = {
    ...
    keyword workspace 8,monitor:eDP-1,                                  on-created-empty: ~/.config/kanshi/start_neovide;\
    ...
    ".config/kanshi/start_neovide" = {
      text = "${pkgs.neovide}/bin/neovide -- --cmd 'cd ~/vaults/Knowledge/'";
      executable = true;
    };
  };
}
